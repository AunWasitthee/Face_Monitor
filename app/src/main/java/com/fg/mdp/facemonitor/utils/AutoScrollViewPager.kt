...
class AutoScrollViewPager : ViewPager {
    ...
    private var autoScrollFactor = 1L
    /** scroll factor for swipe scroll animation, default is 1.0  */
    private var swipeScrollFactor = 1L
    ...
    fun startAutoScroll() {
        isAutoScroll = true
        sendScrollMessage((interval + scroller!!.getDuration() / autoScrollFactor * swipeScrollFactor) )
    }
    ...
    fun setSwipeScrollDurationFactor(scrollFactor: Double) {
        swipeScrollFactor = scrollFactor.toLong()
    }
    ...
    fun setAutoScrollDurationFactor(scrollFactor: Double) {
        autoScrollFactor = scrollFactor.toLong()
    }
    ...
    fun scrollOnce() {
        ...
        var totalCount = adapter?.count ?: 0
        ...
        if (nextItem < 0) {
            if (isCycle) {
                totalCount?.minus(1)?.let { setCurrentItem(it, isBorderAnimation) }
            }
        ...
        override fun handleMessage(msg: Message) {
            ...
            when (msg.what) {
                SCROLL_WHAT -> {
                    ...
                    if (pager != null) {
                        pager!!.scroller!!.setScrollDurationFactor(pager!!.autoScrollFactor.toDouble())
                        pager!!.scrollOnce()
                        pager!!.scroller!!.setScrollDurationFactor(pager!!.swipeScrollFactor.toDouble())
//                        pager!!.sendScrollMessage(pager!!.interval + pager!!.scroller!!.getDuration())
                        pager!!.sendScrollMessage(10000)
                    }
                }
                else -> {
                }
